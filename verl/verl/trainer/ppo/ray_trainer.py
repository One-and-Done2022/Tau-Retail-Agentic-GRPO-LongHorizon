...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.GRPO_CS:
        advantages, returns = core_algos.compute_grpo_cs_advantage(
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=data.batch["response_mask"],
            index=data.non_tensor_batch["uid"],
            step_records=data.non_tensor_batch.get("tool_step_records"),
            norm_adv_by_std_in_grpo=norm_adv_by_std_in_grpo,
            config=config,
        )
        data.batch["advantages"] = advantages
...
class RayPPOTrainer:
    ...
    def _validate(self):
            ...
            if "reasoning_tokens_per_turn" in test_batch.non_tensor_batch:
                for rtpt in test_batch.non_tensor_batch["reasoning_tokens_per_turn"]:
                    if rtpt and len(rtpt) > 0:
                        sample_reasoning_tokens.extend(rtpt)
            if "total_tool_calls" in test_batch.non_tensor_batch:
                sample_tool_calls.extend(test_batch.non_tensor_batch["total_tool_calls"])
            if "total_errors" in test_batch.non_tensor_batch:
                sample_errors.extend(test_batch.non_tensor_batch["total_errors"])
            if "outcome_reward" in test_batch.non_tensor_batch:
        ...
        if sample_reasoning_tokens:
            ...
            metric_dict["val-aux/reasoning_tokens_per_turn/min"] = int(np.min(sample_reasoning_tokens))
        if sample_tool_calls:
            metric_dict["val-aux/tool_calls/mean"] = float(np.mean(sample_tool_calls))
        if sample_errors and sample_tool_calls:
            total_calls = sum(sample_tool_calls)
            total_errors = sum(sample_errors)
            metric_dict["val-aux/error_rate"] = float(total_errors / total_calls) if total_calls > 0 else 0.0
        if sample_outcome_rewards:
        ...
        if len(sample_splits) == len(sample_scores):
            sample_splits_arr = np.array(sample_splits)
            sample_scores_arr = np.array(sample_scores)
            n_val = self.config.actor_rollout_ref.rollout.val_kwargs.n
            for split in ["seen", "unseen"]:
                mask = sample_splits_arr == split
                if mask.sum() > 0:
                    split_scores = sample_scores_arr[mask]
