...
@register_adv_est(AdvantageEstimator.GRPO_CS)
def compute_grpo_cs_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    step_records: Optional[np.ndarray | list] = None,
    epsilon: float = 1e-6,
    norm_adv_by_std_in_grpo: bool = True,
    config: Optional[AlgoConfig] = None,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Add group-relative step credit only to matching assistant tool spans."""
    trajectory_advantages, trajectory_returns = compute_grpo_outcome_advantage(
    ...
    cs_config = config.get("cs_grpo", {}) if config is not None else {}
    beta = float(cs_config.get("beta", 1.0))
    if beta == 0.0:
        return trajectory_advantages, trajectory_returns
    if step_records is None:
        raise ValueError("grpo_cs with beta > 0 requires tool_step_records")
    if len(step_records) != response_mask.shape[0]:
        raise ValueError(
            f"tool_step_records batch size {len(step_records)} does not match response batch {response_mask.shape[0]}"
        )

    response_length = response_mask.shape[1]
    grouped_steps: dict[tuple[Any, int], list[tuple[int, float, int, int]]] = defaultdict(list)
    occupied = torch.zeros_like(response_mask, dtype=torch.bool)

    for row, records in enumerate(step_records):
        if records is None:
            records = []
        if not isinstance(records, (list, tuple, np.ndarray)):
            raise ValueError(f"tool_step_records[{row}] must be a sequence")
        for ordinal, record in enumerate(records):
            if not isinstance(record, dict):
                raise ValueError(f"tool_step_records[{row}][{ordinal}] must be a dict")
            start = record.get("start")
            end = record.get("end")
            reward = record.get("reward")
            if not isinstance(start, (int, np.integer)) or not isinstance(end, (int, np.integer)):
                raise ValueError(f"Non-integer tool span at row {row}, step {ordinal}: {(start, end)}")
            start, end = int(start), int(end)
            if start < 0 or end <= start or end > response_length:
                raise ValueError(f"Out-of-bounds tool span at row {row}, step {ordinal}: {(start, end)}")
            if not bool(torch.all(response_mask[row, start:end] == 1)):
                raise ValueError(f"Tool span covers masked observation/padding at row {row}, step {ordinal}")
            if bool(torch.any(occupied[row, start:end])):
                raise ValueError(f"Overlapping tool spans at row {row}, step {ordinal}")
            try:
                reward_value = float(reward)
            except (TypeError, ValueError) as exc:
                raise ValueError(f"Non-numeric step reward at row {row}, step {ordinal}: {reward}") from exc
            if not np.isfinite(reward_value):
                raise ValueError(f"Non-finite step reward at row {row}, step {ordinal}: {reward_value}")
            occupied[row, start:end] = True
            grouped_steps[(index[row], ordinal)].append((row, reward_value, start, end))

    step_advantages = torch.zeros_like(trajectory_advantages)
    with torch.no_grad():
        for members in grouped_steps.values():
            if len(members) < 2:
                continue
            ...
            if norm_adv_by_std_in_grpo:
                normalized = centered / (rewards.std() + epsilon)
            else:
                normalized = centered
            for value, (row, _, start, end) in zip(normalized, members):
                step_advantages[row, start:end] = value

    advantages = trajectory_advantages + beta * step_advantages
