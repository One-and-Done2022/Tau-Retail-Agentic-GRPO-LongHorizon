...
class AgentData:
    ...
    def __init__(
        ...
        self.pending_tool_call_span: Optional[tuple[int, int]] = None
        self.tool_step_records: list[dict[str, Any]] = []
...
class ToolAgentLoop(AgentLoopBase):
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        output.extra_fields.update({
            ...
            "tool_step_records": agent_data.tool_step_records,
        })
    ...
    async def _handle_generating_state(
        ...
        span_start = len(agent_data.response_mask)
        agent_data.assistant_turns += 1
        agent_data.response_ids = output.token_ids
        agent_data.prompt_ids += agent_data.response_ids
        agent_data.response_mask += [1] * len(agent_data.response_ids)
        span_end = len(agent_data.response_mask)
        if output.log_probs:
        ...
        agent_data.pending_tool_call_span = (span_start, span_end) if agent_data.tool_calls else None
    ...
    async def _handle_processing_tools_state(self, agent_data: AgentData) -> AgentState:
        ...
        response_metadata = []
        response_rewards = []
        for tool_response, tool_reward, metadata in responses:
            ...
            response_rewards.append(float(tool_reward or 0.0))
            response_metadata.append(metadata)

        if responses:
            if agent_data.pending_tool_call_span is None:
                raise RuntimeError("Executed tools without a matching assistant token span")
            agent_data.tool_step_records.append(
                build_tool_step_record(
                    agent_data.pending_tool_call_span,
                    tool_call_names,
                    response_rewards,
                    response_metadata,
                )
            )
            agent_data.pending_tool_call_span = None
