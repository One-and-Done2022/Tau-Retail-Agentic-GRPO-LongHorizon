...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        if role == "actor" and optim_config is not None:
            ...
            # Only pass trainable parameters to optimizer to save memory with LoRA
            trainable_params = [p for p in actor_module_fsdp.parameters() if p.requires_grad]
            actor_optimizer = build_optimizer(trainable_params, optim_config)
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        # The LoRA-disabled pass is used only as the reference policy and its
        # caller discards entropy. Avoid materializing the full-vocabulary
        # entropy tensor for long responses in that pass.
