...
def test_bfloat16_logprobs_chunk_across_token_rows():
    torch.manual_seed(7)
    logits = torch.randn(2, 517, 1024, dtype=torch.bfloat16, requires_grad=True)
    labels = torch.randint(0, logits.shape[-1], logits.shape[:-1])
