...
Usage:
    python scripts/train/grpo/build_grpo_parquet.py \
        --seen-task-ids 0,1,2,3,4,5,6,7 \
        --task-split train \
        --output-train experiments/retail_grpo/train.parquet \
        --output-val experiments/retail_grpo/val.parquet
...
def build_rows(
    ...
    interaction_name = interaction_name or f"tau_bench_{env_name}"
    rows = []
    for idx, tid in enumerate(task_ids):
        rows.append({
            "prompt": [{"role": "system", "content": get_system_prompt(env_name)}],
            "extra_info": {
                "index": idx,
                "task_id": tid,
                "split": split,
...
def main():
    ...
    parser.add_argument("--output-train", default="experiments/retail_grpo/train.parquet")
    parser.add_argument("--output-val", default="experiments/retail_grpo/val.parquet")
    parser.add_argument("--env", choices=("retail",), default="retail")
    parser.add_argument("--task-split", choices=("train", "dev", "test"), default="train")
    parser.add_argument("--interaction-name", default=None)
    parser.add_argument("--num-total-tasks", type=int, default=None)
    ...
    if args.seen_task_ids:
        ...
        if "seen_task_ids" in meta:
            ...
            raise ValueError(f"No seen_task_ids in {meta_path}")

    num_total_tasks = args.num_total_tasks
    if num_total_tasks is None:
        ...
        num_total_tasks = len(env.tasks)

    all_ids = list(range(num_total_tasks))
