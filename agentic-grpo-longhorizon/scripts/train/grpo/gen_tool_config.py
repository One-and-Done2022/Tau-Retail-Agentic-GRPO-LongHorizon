...
Usage:
    python scripts/train/grpo/gen_tool_config.py \
        --env retail \
        --output configs/tool_config/tau_bench_retail_tools.yaml
...
def get_tool_schemas(env_name: str = "retail", task_index: int = 0) -> list[dict]:
...
def main():
    ...
    parser.add_argument("--env", choices=("retail",), default="retail", help="τ-bench env name")
    parser.add_argument("--task-id", type=int, default=0, help="task index for schema extraction")
    parser.add_argument("--output", default="configs/tool_config/tau_bench_retail_tools.yaml")
