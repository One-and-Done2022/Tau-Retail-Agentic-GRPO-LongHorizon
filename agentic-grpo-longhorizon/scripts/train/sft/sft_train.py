...
def main():
    ...
    if use_swanlab:
        ...
        swanlab_cb = SwanLabCallback(
            ...
            experiment_name=cfg["train"].get("run_name", "sft_retail_lora"),
            description=logger_cfg.get("description", "SFT warmup on retail"),
            config={
    ...
    lora_b_delta_l2 = 0.0
    for name, param in model.named_parameters():
        if name not in initial_lora_b:
            continue
        current = param.detach().float().cpu()
        delta = current - initial_lora_b[name]
        delta_norm = float(delta.norm().item())
        current_norm = float(current.norm().item())
        changed_lora_b += int(delta_norm > 0.0)
        lora_b_nonzero += int(current_norm > 0.0)
        lora_b_delta_l2 += delta_norm * delta_norm
    lora_b_delta_l2 = lora_b_delta_l2 ** 0.5
    if changed_lora_b == 0 or lora_b_nonzero == 0 or not torch.isfinite(torch.tensor(lora_b_delta_l2)):
        raise RuntimeError(
            "LoRA update gate failed: "
            f"changed_lora_b={changed_lora_b}, nonzero_lora_b={lora_b_nonzero}, "
            f"delta_l2={lora_b_delta_l2}"
        )
    ...
    summary = {
        ...
        "lora_b_tensors": len(initial_lora_b),
        "changed_lora_b_tensors": changed_lora_b,
        "nonzero_lora_b_tensors": lora_b_nonzero,
        "lora_b_delta_l2": lora_b_delta_l2,
    }
