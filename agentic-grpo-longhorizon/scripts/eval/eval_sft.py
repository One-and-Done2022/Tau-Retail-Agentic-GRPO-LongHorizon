...
def main():
    ...
    base_seed = cfg["eval"].get("seed")
    user_seed_base = cfg["eval"].get("user_seed")

    def policy_factory(task_idx=None, sample_idx=None):
        policy_cfg = dict(cfg["policy"])
        if base_seed is not None and task_idx is not None and sample_idx is not None:
            policy_cfg["seed"] = int(base_seed) + int(task_idx) * 1000 + int(sample_idx)
        return VLLMPolicy(**policy_cfg)
    ...
    report = run_eval(
        ...
        task_ids=cfg["eval"].get("task_ids"),
        policy_seed_base=base_seed,
    ...
    if args.split_file and seen_ids:
        ...
        print(f"\n=== Frozen retail subset ===")
        print(f"  pass^1 = {report.pass_hat_1:.3f}, "
