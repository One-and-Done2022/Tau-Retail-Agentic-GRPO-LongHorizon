...
RUN_ID="${1:-run_001}"
ORCH_DIR="experiments/retail_confirmatory/eval_orchestration/$RUN_ID"
if [[ -e "$ORCH_DIR" ]]; then
...
for CANDIDATE in \
    sft \
    vanilla_step_4 vanilla_step_8 \
    state_step_4 state_step_8 \
    constraint_step_4 constraint_step_8 \
    cs_step_4 cs_step_8; do
    CONFIRMATORY_EVAL_GPU=1 CONFIRMATORY_POLICY_PORT=8012 \
